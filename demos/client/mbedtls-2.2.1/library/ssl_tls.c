...
int mbedtls_ssl_conf_remote_attestation_list( mbedtls_ssl_config *conf,
                                              unsigned char *target_list,
                                              size_t target_list_len,
                                              unsigned char *supported_list,
                                              size_t supported_list_len )
{
    /*
     * The length of target/suppoered attestation list can not
     * larger than the number of available attestation type.
     */
    if (target_list_len > MBEDTLS_SUPPORTED_ATTESTATION_NUM ||
        supported_list_len > MBEDTLS_SUPPORTED_ATTESTATION_NUM) {
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    }

    if (target_list_len != 0) {
        conf->remote_attestation = MBEDTLS_SSL_REMOTE_ATTESTATION_ENABLED;
        conf->target_attestation_list = mbedtls_calloc( 1, target_list_len );
        memcpy(conf->target_attestation_list, target_list, target_list_len);
        conf->target_attestation_list_len = target_list_len;
    } else {
        conf->remote_attestation = MBEDTLS_SSL_REMOTE_ATTESTATION_DISABLED;
        conf->target_attestation_list_len = 0;
    }

    if (supported_list_len != 0) {
        conf->remote_attestation_req = MBEDTLS_SSL_REMOTE_ATTESTATION_ENABLED;
        conf->supported_attestation_list = mbedtls_calloc( 1, supported_list_len );
        memcpy(conf->supported_attestation_list, supported_list, supported_list_len);
        conf->supported_attestation_list_len = supported_list_len;
    } else {
        conf->remote_attestation_req = MBEDTLS_SSL_REMOTE_ATTESTATION_DISABLED;
        conf->supported_attestation_list_len = 0;
    }

    return( 0 );
}

int mbedtls_ssl_conf_remote_attestation( mbedtls_ssl_config *conf,
                                         unsigned char *quote,
				         size_t size ) {
    conf->quote = mbedtls_calloc(1, size);
    memcpy(conf->quote, quote, size);
    conf->quote_size = size;
