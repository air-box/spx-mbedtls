...
static int customized_net_recv(void *ctx, unsigned char *buf, size_t len)
{
  int ret;
  int fd = *(int *)ctx;
...
static int customized_net_send(void *ctx, unsigned char *buf, size_t len)
{
  int ret;
  int fd = *(int *)ctx;
...
while(opt.reconnect_hard > 0)
{
    ...
    unsigned char quote[32];
    memset(quote, 'A', 32);
    mbedtls_ssl_conf_remote_attestation( &conf, quote, 32);*/
#endif
    ...
    if( opt.nbio == 2 )
        ...
        mbedtls_ssl_set_bio( &ssl, &server_fd, customized_net_send, customized_net_recv, NULL );
