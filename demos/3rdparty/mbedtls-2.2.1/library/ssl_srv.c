...
#if defined(MBEDTLS_SSL_REMOTE_ATTESTATION)
static int ssl_parse_remote_attestation_ext( mbedtls_ssl_context *ssl,
                                             unsigned char *buf,
                                             size_t len )
{
    size_t cli_targetlist_len, cli_supportedlist_len;
    size_t srv_targetlist_len, srv_supportedlist_len;
    size_t i, j;
    unsigned char *cli_curr, *srv_curr;

    if( ssl->conf->remote_attestation == MBEDTLS_SSL_REMOTE_ATTESTATION_DISABLED &&
        ssl->conf->remote_attestation_req == MBEDTLS_SSL_REMOTE_ATTESTATION_DISABLED )
    {
        return( 0 );
    }
    ...
    srv_supportedlist_len = ssl->conf->supported_attestation_list_len;
    srv_targetlist_len    = ssl->conf->target_attestation_list_len;

    ssl->supported_attestation_type = MBEDTLS_ATTESTATION_TYPE_NONE;
    ssl->target_attestation_type    = MBEDTLS_ATTESTATION_TYPE_NONE;

    /*
     * Try to find match between the received target list with configured
     * supported list.
     */
    cli_targetlist_len = ( buf[0] << 8 ) | buf[1];
    cli_curr = buf + 2;
    for (i = 0; i < cli_targetlist_len; i++) {
        if (ssl->supported_attestation_type != MBEDTLS_ATTESTATION_TYPE_NONE)
            break;

        srv_curr = ssl->conf->supported_attestation_list;
        for (j = 0; j < srv_supportedlist_len; j++) {
            if (*cli_curr == *srv_curr) {
                ssl->supported_attestation_type = *cli_curr;
                break;
            }
            srv_curr++;
        }
    ...
    if (ssl->supported_attestation_type > MBEDTLS_ATTESTATION_TYPE_UNKNOWN) {
        ssl->supported_attestation_type = MBEDTLS_ATTESTATION_TYPE_UNKNOWN;
    }

    if (ssl->supported_attestation_type != MBEDTLS_ATTESTATION_TYPE_NONE &&
        ssl->supported_attestation_type != MBEDTLS_ATTESTATION_TYPE_UNKNOWN) {
        ssl->handshake->attestation_sent = 1;
    } else {
        ssl->handshake->attestation_sent = 0;
    }


    /*
     * Try to find match between the received supported list with configured
     * target list.
     */
    cli_curr = buf + cli_targetlist_len + 2;
    cli_supportedlist_len = ( cli_curr[0] << 8 ) | cli_curr[1];
    cli_curr += 2;
    for (i = 0; i < cli_supportedlist_len; i++) {
        if (ssl->target_attestation_type != MBEDTLS_ATTESTATION_TYPE_NONE)
            break;

        srv_curr = ssl->conf->target_attestation_list;
        for (j = 0; j < srv_targetlist_len; j++) {
            if (*cli_curr == *srv_curr) {
                ssl->target_attestation_type = *cli_curr;
                break;
            }
            srv_curr++;
        }
    ...
    /* Check if the type if valid */
    if (ssl->target_attestation_type > MBEDTLS_ATTESTATION_TYPE_UNKNOWN) {
        ssl->target_attestation_type = MBEDTLS_ATTESTATION_TYPE_UNKNOWN;
    }

    if (ssl->target_attestation_type != MBEDTLS_ATTESTATION_TYPE_NONE &&
        ssl->target_attestation_type != MBEDTLS_ATTESTATION_TYPE_UNKNOWN) {
        ssl->handshake->attestation_verified = 1;
    } else {
        ssl->handshake->attestation_verified = 0;
    }

    return( 0 );
}
#endif /* MBEDTLS_SSL_REMOTE_ATTESTATION */
...
#if defined(MBEDTLS_SSL_REMOTE_ATTESTATION)
        case MBEDTLS_TLS_EXT_REMOTE_ATTESTATION:
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "found remote attestation extension" ) );

            ret = ssl_parse_remote_attestation_ext( ssl, ext + 4, ext_size );
            if( ret != 0 )
                return( ret );
            break;
#endif /* MBEDTLS_SSL_REMOTE_ATTESTATION */
...
#if defined(MBEDTLS_SSL_REMOTE_ATTESTATION)
static void ssl_write_remote_attestation_ext( mbedtls_ssl_context *ssl,
                                          unsigned char *buf, size_t *olen )
{
    unsigned char *p = buf;
    const unsigned char *end = ssl->out_msg + MBEDTLS_SSL_MAX_CONTENT_LEN;
    ...
    *olen = 0;
    ...
    MBEDTLS_SSL_DEBUG_MSG( 3, ( "server hello, adding remote attestation extension" ) );
     ...
     * 0 . 1    ext identifier
     * 2 . 3    ext length
     * 4        supported attestation type
     * 5        target attestation
     */
    *p++ = (unsigned char)( ( MBEDTLS_TLS_EXT_REMOTE_ATTESTATION >> 8 ) & 0xFF );
    *p++ = (unsigned char)( ( MBEDTLS_TLS_EXT_REMOTE_ATTESTATION      ) & 0xFF );

    *p++ = 0x00;
    *p++ = 2;

    *p++ = (unsigned char) ( (ssl->supported_attestation_type )   & 0xFF );
    *p++ = (unsigned char) ( (ssl->target_attestation_type )      & 0xFF );

    /* total len = 1 (supported_type) +  1 (target_type) +
     *             2 (ext_type) + 2 (ext_len)               */
    *olen = 6;
}
#endif /* MBEDTLS_SSL_REMOTE_ATTESTATION */
...
#if defined(MBEDTLS_SSL_REMOTE_ATTESTATION)
    ssl_write_remote_attestation_ext( ssl, p + 2 + ext_len, &olen );
    ext_len += olen;
#endif
...
#if defined(MBEDTLS_SSL_REMOTE_ATTESTATION)
static int ssl_write_remote_attestation( mbedtls_ssl_context *ssl )
{
    /* TODO: Get remote attestation based on the type */
    ssl->handshake->attestation_sent = 0;
    return ( 0 );
}
#endif
...
#if defined(MBEDTLS_SSL_REMOTE_ATTESTATION)
static int ssl_parse_remote_attestation( mbedtls_ssl_context *ssl )
{
    int ret;

    /* TODO: verify attestation based on the type. */
    if ((ret = mbedtls_ssl_read_record( ssl ) ) != 0)
    {
        MBEDTLS_SSL_DEBUG_RET( 1, "mbedtls_ssl_read_record", ret );
        return( ret );
    ...
    if( ssl->in_msgtype != MBEDTLS_SSL_MSG_HANDSHAKE ||
        ssl->in_msg[0] != MBEDTLS_TLS_EXT_REMOTE_ATTESTATION )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad remote attestation message" ) );
        return( MBEDTLS_ERR_SSL_BAD_HS_CERTIFICATE_VERIFY );
    }
...
#if defined(MBEDTLS_SSL_REMOTE_ATTESTATION)
            if (ssl->handshake->attestation_sent != 0)
                ret = ssl_write_remote_attestation( ssl );
            else
#endif
...
#if defined(MBEDTLS_SSL_REMOTE_ATTESTATION)
        if (ssl->handshake->attestation_verified != 0)
            ret = ssl_parse_remote_attestation( ssl );
        else
#endif
